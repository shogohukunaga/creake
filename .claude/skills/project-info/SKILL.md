...
| `serv-icons1〜3.webp` | アイコンスプライトシート（**現在はCSSから未参照**。個別webpの切り出し元として保持） |
| `serv-ic-why01〜04.webp` / `serv-ic-reco1,2,5.webp` | 制作代行/おすすめの丸アイコン（スプライトからPILで個別切り出し。`.l-serv-ic--why01〜04/reco1〜5`） |
...
- 共通の青いタイトル箱 = `.c-serv-ttl`。丸アイコン（why01〜04/reco1〜5）は**個別webp**を `background: url(...) no-repeat center center/cover;` のショートハンド1行で敷く（**スプライトの%切り出し・longhand分割は禁止**）。recoは各アイコンごとにFigma寸法のwidth/heightを個別指定（reco1/2/5=124×120、reco3=124×110、reco4=124×104。SPは80×78/71/67）。
