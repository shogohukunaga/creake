...
### background画像はショートハンド1行で書く（スプライト%切り出し禁止）

背景画像は `background:` ショートハンドにまとめる。`background-image` / `background-size` / `background-position` の longhand 分割は書かない。

```scss
// NG（longhandを並べる）
.l-serv-ic--reco4 {
  background-image: url(../img/pc/serv-ic-reco4.webp);
...
// NG（スプライトシートの%切り出し）
.l-serv-ic--reco5 {
  background-image: url(../img/pc/serv-icons3.webp);
  background-size: 665.88% 608.01%;
  background-position: 26.93% 30.93%;
}

// OK
.l-serv-ic--reco4 {
  background: url(../img/pc/serv-ic-reco4.webp) no-repeat center center/cover;
}
```

- **スプライトシートを `background-size/position` の%指定で切り出す書き方は禁止。** アイコンはPIL等で個別画像（webp）に切り出してから使う。
- パスはコンパイル後の `public/assets/css/` 基準で `../img/pc/xxx.webp`（1階層上）。`../../` は届かないので書かない。
- コンテナごとにサイズが違うアイコンは、共通クラスで統一サイズにせず**各モディファイアにFigma寸法の `width`/`height` を個別指定**する。
